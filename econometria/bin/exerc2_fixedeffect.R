...
## Pooled OLS ----
ols.0 <- lm(model1, pdata)
pooled.ols.1 <- plm(model1, data = pdata, model = "within", effect = "twoways")
pooled.ols.2 <- plm(model2, data = pdata, model = "within", effect = "twoways")

# 6. CHECK MODELS ----
summary(ols.0)
summary(pooled.ols.1)

stargazer(pooled.ols.1, pooled.ols.2, type = "html", report = "vcsp*", out = "./econometria/exerc2_pooledols.html")
stargazer(ols.0, pooled.ols.1, pooled.ols.2, type = "text", report = "vcsp*")
